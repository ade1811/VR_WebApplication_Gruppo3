...
from functions.func import send_email, change_name_if_exist, get_popolazione, pagamenti, verifica_pagamento
...
def login():
    try:
        ...
        if ok:
            ...
            return {"token": access_token, "anagrafica": message["anagrafica"], "immagine": message["immagine"], "email": email}, 200
        else:
            return message, 400
...
def signin():
    try:
        ...
        if ok:
            access_token = create_access_token(identity=f"{message['token']}_{message['isPromoter']}")
            return {"token": access_token, "role": role, "anagrafica": message["anagrafica"], "email": email}, 200
        else:
            return message, 400
    except Exception as e:
        return {"message": str(e)}, 500
...
@app.route("/api/modifyEvent", methods = ["PUT"])
@jwt_required()
def modifyEvent():
    identity = get_jwt_identity()
    id, isPromoter = identity.split('_')
...
def addPacchetto(eventoID):
    ...
    if isPromoter == "True":
        ...
        for pacchetto in pacchetti:
            ...
            Promoter(db_config, id).setPacchetti(eventoID, pacchetto["title"], pacchetto["price"], pacchetto["description"])
...
def modifyProfile():
    try:
        # Utilizza request.form per i dati inviati con FormData
        form_data = request.form.to_dict()
        nome = form_data.get("nome") or None
        cognome = form_data.get("cognome") or None
        ...
        if "file" in request.files:
            ...
            # Salvataggio file (come nel tuo codice originale)
            save_path = os.path.join("C:/Users/ade59/Documents/webapp/ImmaginiDB", filename)
            if os.path.exists(save_path):
                filename, save_path = change_name_if_exist(file)
            file.save(save_path)
        ...
        # Ottieni l'identità JWT
        identity = get_jwt_identity()
        id, isPromoter = identity.split('_')
    except Exception as e:
        return {"message": str(e)}, 500

    return User(db_config).modifyProfile(id, isPromoter, nome, cognome, email, filename)
...
def createTicket():
    try:
        ...
        sessionid = pagamenti(data)
        if sessionid:
            return {"message": "Controlla le mail", "id": sessionid}, 200
        else:
...
@app.route("/api/verify/<sessionID>", methods=["POST"])
@jwt_required()
def verifyPag(sessionID):
    try:
        identity = get_jwt_identity()
        idPromoter, isPromoter = identity.split('_')
        data = request.json

        if verifica_pagamento(sessionID):
            t = Ticket(db_config)
            t.createTicket(idPromoter, data)
            return {"message": "Ticket creati con successo"}, 200
    except Exception as e:
...
def delete():
    try:
        ...
        return User(db_config).delete(id, role)
    except Exception as e:
        return {"message": e}
    
@app.route("/api/getMyTickets", methods = ["GET"])
@jwt_required()
def myTicket():
    identity = get_jwt_identity()
    id, isPromoter = identity.split('_') 
    return User(db_config).getMyTicket(id)   

@app.route("/api/getEventDay", methods = ["POST"])
@jwt_required()
def getEventDay():
    try:
        identity = get_jwt_identity()
        ...
        return Promoter(db_config, id).getEventDay(data)
    except Exception as e:
        return {"message": e}
    
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5328, debug=True, threaded = True )