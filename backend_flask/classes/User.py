...
class User:
    ...
    def logIn(self, role: str):
        try:
            ...
            query = f"SELECT password, {role}ID FROM {role} WHERE email = %s AND isActive"
    ...
    def getProfile(self, id, isPromoter):
        try:
            ...
            return {"id": id, "numEventi" : num_events, "userType": userType, 
                    "anagrafica": self.__getUser(userType, id), "immagine": self.__getProfileImage(userType, id)}
    ...
    def modifyProfile(self, id, isPromoter, nome, cognome, email, filename):
        try:
            self._ensure_connection()
    ...
    def delete(self, id, role):
        self._ensure_connection()
        query = f"UPDATE {role} SET isActive = 0 WHERE {role}ID = %s"
        self.cursor.execute(query, (id,))
        self.engine.commit()
        self.engine.close()

        return {"message": "Ciao, ci mancherai"}, 200
    
    def getMyTicket(self, id):
        query = "SELECT * FROM ticket where userID = %s"
        data = MySQL_to_JSON(query, (id, ), self.engine)
