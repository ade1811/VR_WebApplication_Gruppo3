...
class Promoter():
    ...
    def dashboard(self) -> tuple:
        ...
        if num_amount > 0:
            ...
            query = """SELECT e.titolo FROM ticket t JOIN 
            pacchetto p ON t.pacchettoID = p.pacchettoID JOIN evento e ON p.eventoID = e.eventoID
            WHERE e.promoterID = %s AND e.data = (SELECT MAX(data) FROM evento WHERE promoterID = %s)
            GROUP BY e.titolo;"""
            self.cursor.execute(query, (self.id, self.id))
            titolo = self.cursor.fetchall()
        ...
        data = {
            "data": {
                ...
                "profit": {
                    "title": f"Ricavo dell'ultimo evento",
    ...
    def createBouncer(self, email: str, eventID: int):
        ...
        try:
            ...
            if self.cursor.fetchone()[0] == 0:
                ...
                self.cursor.execute("SELECT bouncerID FROM BOUNCER WHERE email = %s", (email,))
            else:
                self.cursor.execute("SELECT bouncerID FROM BOUNCER WHERE email = %s", (email,))

            query = "INSERT INTO bouncer_lavorare_evento (bouncerID, eventoID) VALUES (%s, %s)"
            self.cursor.execute(query, (self.cursor.fetchone()[0], eventID))
    ...
    def getEventDay(self, data):
        data = datetime.strptime(data, "%d/%m/%Y").strftime("%Y-%m-%d")

        if data == None:
            return {"eventi": []}, 200

        query = "SELECT * FROM evento WHERE data = %s"
        eventi = MySQL_to_JSON(query=query, params=(data,), engine=self.engine)
        for evento in eventi["data"]:
            try:
                lat, lon = evento["luogo"].split(",")
                evento["luogo"] = get_indirizzo(lat.strip(), lon.strip())
            except ValueError:
